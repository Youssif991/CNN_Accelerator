`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// Engineer: Youssef
//
// Create Date: 08/27/2026
// Design Name: Gate-Level Functional Check
// Module Name: tb_netlist_check
// Tool Versions: Vivado 2025.2
// Description: Quick functional check of the synthesized accelerator_top
//              netlist (post-route, primitives). Runs one frame with a
//              deterministic image/kernel and compares all 900 outputs
//              against an inline convolution golden model, proving the
//              pipelined + memory-inferred implementation is functionally
//              equivalent to the RTL.
//
// Dependencies: accelerator_top netlist (synth_out/accelerator_top_netlist.v)
//
// Revision:
// Revision 0.01 - File Created
// Additional Comments:
//
//////////////////////////////////////////////////////////////////////////////////

module tb_netlist_check;

    // Parameters
    localparam N = 3;
    localparam IMAGE_WIDTH = 32;
    localparam IMAGE_HEIGHT = 32;
    localparam PIXEL_WIDTH = 8;
    localparam COEFF_WIDTH = 8;
    localparam OUT_WIDTH = 16;
    localparam TOTAL_PIXELS = IMAGE_WIDTH * IMAGE_HEIGHT;
    localparam PIX_ADDR_WIDTH = $clog2(TOTAL_PIXELS);
    localparam OUT_IMAGE_WIDTH = IMAGE_WIDTH - N + 1;
    localparam OUT_TOTAL = OUT_IMAGE_WIDTH * OUT_IMAGE_WIDTH;
    localparam OUT_ADDR_WIDTH = $clog2(OUT_TOTAL);
    localparam PROD_WIDTH = PIXEL_WIDTH + COEFF_WIDTH + 2;
    localparam SUM_WIDTH = PROD_WIDTH + $clog2(N*N);
    localparam BITS_DROPPED = SUM_WIDTH - OUT_WIDTH;
    localparam SAT_MAX = (1 << (OUT_WIDTH-1)) - 1;
    localparam SAT_MIN = -(1 << (OUT_WIDTH-1));

    // DUT interface
    reg clk_i;
    reg rst_n_i;
    reg start_i;
    reg buf_sel_i;
    reg kernel_wr_valid_i;
    reg [COEFF_WIDTH-1:0] kernel_wr_data_i;
    reg img_wr_valid_i;
    reg [PIX_ADDR_WIDTH:0] img_wr_addr_i;
    reg [PIXEL_WIDTH-1:0] img_wr_data_i;
    reg [OUT_ADDR_WIDTH-1:0] res_rd_addr_i;
    wire busy_o;
    wire done_o;
    wire [2:0] state_o;
    wire [OUT_WIDTH-1:0] res_rd_data_o;

    // Test infrastructure
    integer errors = 0;
    integer w;  // image write index
    integer t;  // kernel tap
    integer a;  // output address
    integer r;  // output row
    integer c;  // output col
    integer pix_idx;  // input pixel index for the golden model
    integer pix_val;  // input pixel value for the golden model
    reg signed [COEFF_WIDTH-1:0] ref_kernel [0:N*N-1];
    reg signed [SUM_WIDTH-1:0] ref_sum;
    reg signed [SUM_WIDTH-1:0] ref_shifted;
    reg signed [OUT_WIDTH-1:0] ref_out;

    // Module instantiation
    accelerator_top dut (
        .clk_i            (clk_i),
        .rst_n_i          (rst_n_i),
        .start_i          (start_i),
        .buf_sel_i        (buf_sel_i),
        .kernel_wr_valid_i(kernel_wr_valid_i),
        .kernel_wr_data_i (kernel_wr_data_i),
        .img_wr_valid_i   (img_wr_valid_i),
        .img_wr_addr_i    (img_wr_addr_i),
        .img_wr_data_i    (img_wr_data_i),
        .res_rd_addr_i    (res_rd_addr_i),
        .busy_o           (busy_o),
        .done_o           (done_o),
        .state_o          (state_o),
        .res_rd_data_o    (res_rd_data_o)
    );

    // Clock generation: free-running 20 ns period (50 MHz)
    initial begin : clock
        clk_i = 0;
        forever #10 clk_i = ~clk_i;
    end

    // Task: write one pixel (host-paced pulse)
    task write_pixel;
        input [PIX_ADDR_WIDTH:0] addr;
        input [PIXEL_WIDTH-1:0] data;
        begin
            img_wr_addr_i  = addr;
            img_wr_data_i  = data;
            img_wr_valid_i = 1;
            @(negedge clk_i);
            img_wr_valid_i = 0;
        end
    endtask

    // Task: write one kernel coefficient (host-paced pulse)
    task write_kernel;
        input [COEFF_WIDTH-1:0] coef;
        begin
            kernel_wr_data_i  = coef;
            kernel_wr_valid_i = 1;
            @(negedge clk_i);
            kernel_wr_valid_i = 0;
        end
    endtask

    // Debug probe: trace the first few output writes (hierarchical refs)
    initial begin : probe
        integer writes;
        writes = 0;
        while (writes < 6) begin
            @(posedge clk_i);
            if (dut.result_valid_p) begin
                writes = writes + 1;
                $display("WRITE %0d t=%0t addr=%0d data=%0d", writes, $time, dut.out_addr,
                         $signed(dut.result));
            end
        end
    end

    // Test procedure: one deterministic frame, then a full golden check
    initial begin : test
        // Drive all inputs low and assert reset
        start_i = 0;
        buf_sel_i = 0;
        kernel_wr_valid_i = 0;
        kernel_wr_data_i = 0;
        img_wr_valid_i = 0;
        img_wr_addr_i = 0;
        img_wr_data_i = 0;
        res_rd_addr_i = 0;
        rst_n_i = 0;

        @(negedge clk_i);
        rst_n_i = 1;
        @(negedge clk_i);

        // Load a deterministic (non-degenerate) image into buffer 0
        for (w = 0; w < TOTAL_PIXELS; w = w + 1) begin
            write_pixel({1'b0, w[PIX_ADDR_WIDTH-1:0]}, (w*13 + 5) & 255);
        end

        // Kernel: a small signed pattern
        ref_kernel[0] = 1; ref_kernel[1] = -2; ref_kernel[2] = 3;
        ref_kernel[3] = 4; ref_kernel[4] = 5; ref_kernel[5] = -6;
        ref_kernel[6] = 7; ref_kernel[7] = 8; ref_kernel[8] = -9;

        // Start the frame and load the kernel
        start_i = 1;
        @(negedge clk_i);
        start_i = 0;
        for (t = 0; t < N*N; t = t + 1) write_kernel(ref_kernel[t]);

        // Wait for completion
        while (!done_o) @(negedge clk_i);

        // Golden check of all outputs (registered readback)
        for (a = 0; a < OUT_TOTAL; a = a + 1) begin
            r = a / OUT_IMAGE_WIDTH;
            c = a % OUT_IMAGE_WIDTH;
            ref_sum = 0;
            for (t = 0; t < N*N; t = t + 1) begin
                pix_idx = (r + t/N)*IMAGE_WIDTH + c + t%N;
                pix_val = (pix_idx*13 + 5) & 255;
                ref_sum = ref_sum + $signed({1'b0, pix_val[7:0]}) * ref_kernel[t];
            end
            ref_shifted = ref_sum + (1 << (BITS_DROPPED-1));
            ref_shifted = ref_shifted >>> BITS_DROPPED;
            if (ref_shifted > SAT_MAX) begin
                ref_out = SAT_MAX;
            end else if (ref_shifted < SAT_MIN) begin
                ref_out = SAT_MIN;
            end else begin
                ref_out = $signed(ref_shifted[OUT_WIDTH-1:0]);
            end
            res_rd_addr_i = a;
            @(negedge clk_i);
            @(negedge clk_i);
            if (res_rd_data_o !== ref_out) begin
                errors = errors + 1;
                $display("FAIL t=%0t: out[%0d] = %0d expected %0d", $time, a, res_rd_data_o,
                         ref_out);
            end
        end

        #20;

        if (errors == 0) $display(" TEST PASSED — all checks matched");
        else $display(" TEST FAILED — %0d mismatches found", errors);

        $finish;
    end

endmodule
